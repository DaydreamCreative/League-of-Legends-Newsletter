...
def build_recap(league, season, hist):
    ...
    if not rw:
        ...
        lines.append(f"\nRECAPPING COMPLETED WEEK {rw}. FINAL RESULTS:")
        for a, b in _week_pairs(rw):
            ...
            lines.append("  " + ia.get("team", "") + " top: " + ", ".join(f"{n} {p}" for n, p in top_players(a)))
            lines.append("  " + ib.get("team", "") + " top: " + ", ".join(f"{n} {p}" for n, p in top_players(b)))
            if ia.get("user_id") and ib.get("user_id"):
                lines.append("  history: " + h2h_line(hist, ia["user_id"], ib["user_id"]))
        lines.append("\nSTANDINGS:")
...
def build_preview(league, season, hist):
    ...
    lines = [f"LEAGUE: {league.get('name')}  SEASON: {season}  EDITION: THURSDAY PREVIEW",
             f"\nPREVIEWING WEEK {pw} (games have NOT been played yet, no scores exist):"]
...
Structure:
  ...
  <h3>The Recaps</h3> for EACH completed game, an <h3> subhead with the final score
    and a <p> on how it played out: the decisive performers by name, the hero and the
    goat, and a rivalry or history nod when there is juice. Cover every game.
  <h3>Power Rankings</h3> an <ol> of all teams, one line barb each.
  <h3>Looking Ahead</h3> a <p> teasing next week's schedule from the briefing, circling
    the one or two matchups worth watching. Just a tease, not full previews.
  <h3>The Back Page</h3> a closing bit with the usual bias.
Ground every score and name in the briefing."""
...
Ground records and history in the briefing; the predictions are yours to make."""
