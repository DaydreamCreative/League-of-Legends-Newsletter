...
def call_claude(briefing):
    # Remove ALL whitespace, not just the ends. A key pasted as two lines has a
    # break in the middle, which is what caused the "Illegal header value" error;
    # .strip() alone cannot fix that, but this can. Real keys never contain spaces.
...
def main():
    ...
    print(f"Building edition for league {LEAGUE_ID}, week {week}")
