...
# Your Substack feed. Fill this in (or set the SUBSTACK_FEED env var) and the reporter
# reads your recent PUBLISHED, edited posts each run: it mirrors your voice and builds
# on past storylines instead of repeating them. Only public (free) posts are readable.
SUBSTACK_FEED = (os.environ.get("SUBSTACK_FEED")
                 or "https://matthewwaters612.substack.com/feed").strip()
...
def substack_context(max_posts=6, per_post_chars=1100):
    """Recent published editions from your Substack, as voice samples and continuity.
    Returns '' if no feed is set or anything fails, so it never breaks a run."""
    if not SUBSTACK_FEED:
        return ""
    try:
        r = _session.get(SUBSTACK_FEED, timeout=30)
        r.raise_for_status()
        root = ET.fromstring(r.content)
        content_tag = "{http://purl.org/rss/1.0/modules/content/}encoded"
        blocks = []
        for it in root.findall(".//item")[:max_posts]:
            title = (it.findtext("title") or "").strip()
            date = (it.findtext("pubDate") or "").strip()
            body = it.findtext(content_tag) or it.findtext("description") or ""
            text = _strip_html(body)[:per_post_chars]
            if title or text:
                blocks.append(f"### {title} ({date})\n{text}")
        return "\n\n".join(blocks)
    except Exception as e:
        print(f"substack_context skipped: {e!r}")
        return ""
...
def _week_pairs(week):
    ...
    for m in ms:
        if m.get("matchup_id") is not None:
            pairs[m["matchup_id"]].append(m)
    return [p for p in pairs.values() if len(p) == 2]


def _completed_week():
    """Highest week that already has real scores (the last one that finished)."""
    cur = int(get_state().get("week") or 1)
    for w in range(min(cur, 18), 0, -1):
        ms = get_matchups(LEAGUE_ID, w)
        if ms and any((m.get("points") or 0) > 0 for m in ms):
            return w
    return None


def _standings(owners, rosters):
    rows = []
    for rid, s in rosters.items():
        pts = (s.get("fpts", 0) or 0) + (s.get("fpts_decimal", 0) or 0) / 100
        rows.append({"team": owners.get(rid, {}).get("team"), "w": s.get("wins", 0),
                     "l": s.get("losses", 0), "pf": round(pts, 2)})
    rows.sort(key=lambda x: (x["w"], x["pf"]), reverse=True)
    return rows


def _champ_and_lore(hist):
    lines = []
    if hist["champions"]:
        lines.append("\nSLEEPER ERA CHAMPIONS:")
        for c in sorted(hist["champions"], key=lambda x: x["season"] or ""):
    ...
    lore = _read_lore()
    if lore.strip():
        lines.append("\nLEAGUE LORE (history and running storylines):\n" + lore)
    sub = substack_context()
    if sub:
        lines.append("\nMY RECENT PUBLISHED EDITIONS (from my Substack; these are my own "
                     "edited, posted versions, so mirror this voice and vocabulary, and "
                     "build on these storylines without repeating them):\n" + sub)
    return lines


def _lineup(m):
    """(starters, bench) for a matchup, each a list of (name, position, points).
    Bench is sorted highest points first so 'points left on the bench' is obvious."""
    starters = m.get("starters") or []
    sp = m.get("starters_points") or []
    pp = m.get("players_points") if isinstance(m.get("players_points"), dict) else {}
    allp = m.get("players") or list(starters)
    s_list = [(player_name(pid), player_position(pid),
               round((sp[i] if i < len(sp) else pp.get(pid, 0)) or 0, 2))
              for i, pid in enumerate(starters)]
    bench = [pid for pid in allp if pid not in starters]
    b_list = [(player_name(pid), player_position(pid), round(pp.get(pid, 0) or 0, 2))
              for pid in bench]
    b_list.sort(key=lambda x: x[2], reverse=True)
    return s_list, b_list


def _standings_with_movers(owners, rosters, rw):
    """Current standings with each team's rank movement versus before week rw."""
    wk_pts, wk_res = {}, {}
    for a, b in _week_pairs(rw):
        pa, pb = a.get("points") or 0, b.get("points") or 0
        wk_pts[a["roster_id"]], wk_pts[b["roster_id"]] = pa, pb
        if pa > pb:
            wk_res[a["roster_id"]], wk_res[b["roster_id"]] = "W", "L"
        elif pb > pa:
            wk_res[a["roster_id"]], wk_res[b["roster_id"]] = "L", "W"
        else:
            wk_res[a["roster_id"]] = wk_res[b["roster_id"]] = "T"

    def pf_of(s):
        return (s.get("fpts", 0) or 0) + (s.get("fpts_decimal", 0) or 0) / 100

    prev = [{"rid": rid,
             "w": s.get("wins", 0) - (1 if wk_res.get(rid) == "W" else 0),
             "l": s.get("losses", 0) - (1 if wk_res.get(rid) == "L" else 0),
             "pf": pf_of(s) - wk_pts.get(rid, 0)} for rid, s in rosters.items()]
    prev.sort(key=lambda x: (x["w"], x["pf"]), reverse=True)
    prev_rank = {row["rid"]: i + 1 for i, row in enumerate(prev)}

    cur = [{"rid": rid, "team": owners.get(rid, {}).get("team"),
            "w": s.get("wins", 0), "l": s.get("losses", 0), "pf": round(pf_of(s), 2)}
           for rid, s in rosters.items()]
    cur.sort(key=lambda x: (x["w"], x["pf"]), reverse=True)
    for i, row in enumerate(cur):
        row["rank"] = i + 1
        row["move"] = prev_rank.get(row["rid"], i + 1) - (i + 1)
    return cur


def build_recap(league, season, hist):
    owners = owner_map(LEAGUE_ID)
    rosters = {r["roster_id"]: (r.get("settings") or {}) for r in get_rosters(LEAGUE_ID)}
    rw = _completed_week()
    lines = [f"LEAGUE: {league.get('name')}  SEASON: {season}  EDITION: TUESDAY RECAP"]
    if not rw:
        lines.append("No games have been played yet this season, so there is nothing "
                     "to recap. Set the table for Week 1 and tease the schedule instead.")
        nw, title, slug = 1, f"{season} Kickoff Recap", f"recap-{season}-wk0"
    else:
        lines.append(f"\nRECAPPING COMPLETED WEEK {rw}. RESULTS WITH FULL LINEUPS:")
        for a, b in _week_pairs(rw):
            ia, ib = owners.get(a["roster_id"], {}), owners.get(b["roster_id"], {})
            pa, pb = round(a.get("points") or 0, 2), round(b.get("points") or 0, 2)
            winner = ia.get("team") if pa >= pb else ib.get("team")
            lines.append(f"\n- {ia.get('team')} {pa} vs {pb} {ib.get('team')} (winner: {winner})")
            for m, info in ((a, ia), (b, ib)):
                s_list, b_list = _lineup(m)
                lines.append(f"  {info.get('team')} started: "
                             + ", ".join(f"{n} {pos} {p}" for n, pos, p in s_list))
                if b_list and any(p for _, _, p in b_list):
                    lines.append(f"  {info.get('team')} best bench options left unused: "
                                 + ", ".join(f"{n} {pos} {p}" for n, pos, p in b_list[:4]))
            if ia.get("user_id") and ib.get("user_id"):
                lines.append("  history: " + h2h_line(hist, ia["user_id"], ib["user_id"]))
        nw, title, slug = rw + 1, f"Week {rw} Recap, {season}", f"recap-{season}-wk{rw}"
        lines.append("\nCURRENT STANDINGS (movement is versus before this week):")
        for row in _standings_with_movers(owners, rosters, rw):
            mv = row["move"]
            tag = f"up {mv}" if mv > 0 else (f"down {-mv}" if mv < 0 else "no change")
            lines.append(f"{row['rank']}. {row['team']} {row['w']} and {row['l']} "
                         f"({row['pf']} pts) [{tag}]")
    up = _week_pairs(nw)
    if up:
        lines.append(f"\nNEXT UP, WEEK {nw} SCHEDULE (tease only):")
        for a, b in up:
            ia, ib = owners.get(a["roster_id"], {}), owners.get(b["roster_id"], {})
            sa, sb = rosters.get(a["roster_id"], {}), rosters.get(b["roster_id"], {})
            lines.append(f"- {ia.get('team')} ({sa.get('wins',0)} and {sa.get('losses',0)}) "
                         f"vs {ib.get('team')} ({sb.get('wins',0)} and {sb.get('losses',0)})")
    lines += _champ_and_lore(hist)
    return "\n".join(lines), title, slug


def build_preview(league, season, hist):
    owners = owner_map(LEAGUE_ID)
    rosters = {r["roster_id"]: (r.get("settings") or {}) for r in get_rosters(LEAGUE_ID)}
    pw = (_completed_week() or 0) + 1
    lines = [f"LEAGUE: {league.get('name')}  SEASON: {season}  EDITION: THURSDAY PREVIEW",
             f"\nPREVIEWING WEEK {pw} (games NOT played yet; lineups below are what each "
             f"manager currently has set, for start and sit critique):"]
    pairs = _week_pairs(pw)
    if not pairs:
        lines.append("No matchups are scheduled for this week yet.")
    for a, b in pairs:
        ia, ib = owners.get(a["roster_id"], {}), owners.get(b["roster_id"], {})
        sa, sb = rosters.get(a["roster_id"], {}), rosters.get(b["roster_id"], {})
        lines.append(f"\n- {ia.get('team')} ({sa.get('wins',0)} and {sa.get('losses',0)}) "
        ...
        for m, info in ((a, ia), (b, ib)):
            s_list, b_list = _lineup(m)
            lines.append(f"  {info.get('team')} set starters: "
                         + ", ".join(f"{n} ({pos})" for n, pos, _ in s_list))
            if b_list:
                lines.append(f"  {info.get('team')} bench: "
                             + ", ".join(f"{n} ({pos})" for n, pos, _ in b_list))
        if ia.get("user_id") and ib.get("user_id"):
            lines.append("  history: " + h2h_line(hist, ia["user_id"], ib["user_id"]))
    lines.append("\nSTANDINGS:")
    for i, s in enumerate(_standings(owners, rosters), 1):
        lines.append(f"{i}. {s['team']} {s['w']} and {s['l']} ({s['pf']} pts)")
    lines += _champ_and_lore(hist)
    return "\n".join(lines), f"Week {pw} Preview, {season}", f"preview-{season}-wk{pw}"
...
# Personas
# ----------------------------------------------------------------------------
COMMON_BASE = """You are Matthew Waters, founding-era manager and self-appointed beat
...
- You are the winningest manager in league history and a three time champion
  (2019, 2021, 2023).
...
- If the briefing includes MY RECENT PUBLISHED EDITIONS, treat them as the truest
  sample of my voice: match their phrasing, favorite bits, and rhythm, and continue
  the storylines they set up rather than starting from scratch.
...
BIAS_HOMER = """BIAS
- You are a shameless homer. Cover your own team (Matt W) with suspicious
  generosity and openly joke about the conflict of interest, but still cover
  everyone fairly enough to be readable. The bias is the running gag."""

BIAS_SLIGHT = """BIAS
- Keep the bias very slight this time. Be a credible, mostly objective analyst and
  judge every team on the merits. You may give your own team (Matt W) the faintest
  benefit of the doubt in a genuine coin flip, but nothing blatant. Drop the
  shameless homer act for this edition; the restraint is the joke."""
...
RECAP_PERSONA = """THIS EDITION: the Tuesday recap, out after the week's games finish.
Structure:
  <h2> a headline for the week that just ended
  a <p> lede on the biggest result or story
  <h3>The Recaps</h3> for EACH completed game, an <h3> subhead with the final score,
    then a <p> on how it played out. Weave in, for BOTH teams: the standout HIGH
    performers by name, the players who cratered and LOST it for their team, and any
    LINEUP decisions worth judging. Call out mistakes (big points left on the bench,
    a bad start or sit) and credit good calls (a gutsy start that paid off). Use the
    started and bench numbers in the briefing to back every claim. Cover every game.
  <h3>Standings and Movers</h3> lay out the CURRENT standings in order, then call out
    the biggest risers and fallers versus last week, using the movement tags, and say
    why they moved.
  <h3>Looking Ahead</h3> a short <p> teasing next week's schedule.
  <h3>The Back Page</h3> a closing bit with the usual bias.
Ground every score, name, and lineup point in the briefing."""

PREVIEW_PERSONA = """THIS EDITION: the Thursday preview, out before Thursday Night
Football, before any of this week's games are played. There are NO scores yet, so do
not invent any. Structure:
  <h2> a headline for the week ahead
  a <p> lede on the week's biggest storyline
  <h3>The Matchups</h3> for EACH game this week, an <h3> subhead (Team vs Team) and a
    <p> on the stakes, the players to watch, the head to head history, and YOUR pick
    to win. Where a manager's currently set lineup looks off, SAY SO: call out a
    questionable start, or a clearly better option sitting on their bench, using the
    set starters and bench in the briefing and your football knowledge. Be opinionated
    but fair, and do not force a take when the lineup looks fine. Cover every game.
  <h3>Locks and Upset Watch</h3> a short <ul>: your safest pick, your upset special,
    and the closest game to call.
  <h3>The Back Page</h3> a bold prediction for the week, biased toward yourself.
Ground records, lineups, and history in the briefing; the picks and start/sit takes
are yours to make."""

PERSONAS = {"weekly": WEEKLY, "preseason": PRESEASON,
            "recap": RECAP_PERSONA, "preview": PREVIEW_PERSONA}
...
def _forced_or_die(pairs, person_of, label):
    if not pairs:
        return None                      # no forced list: optimizer is intended
    resolved = _resolve_forced(pairs, person_of)
    if resolved is not None:
        return resolved
    name_to_uid = {(nm or "").strip().lower(): u for u, nm in person_of.items() if u is not None}
    notfound = sorted({n for pr in pairs for n in pr if n.strip().lower() not in name_to_uid})
    seen = ", ".join(sorted(str(person_of[u]) for u in person_of if u is not None))
    detail = (", ".join(notfound) if notfound
              else "(all names matched, but the set does not cover every manager exactly once)")
    raise SystemExit(
        f"Could not apply the {label} rivalry matchups, so I stopped instead of "
        f"publishing a wrong schedule. Names from your list I could not find: {detail}. "
        f"Managers I can see on Sleeper: {seen}. Fix REAL_NAMES so every handle maps to "
        f"the right person, then rerun.")


def _perfect_matchings(edge_set, verts):
    if not verts:
        yield []
        return
    v = verts[0]
    for i in range(1, len(verts)):
        u = verts[i]
        if frozenset((v, u)) in edge_set:
            rest = verts[1:i] + verts[i + 1:]
            for m in _perfect_matchings(edge_set, rest):
                yield [(v, u)] + m


def _factorize(edge_set, verts, need, guard):
    if need == 0:
        return [] if not edge_set else None
    for pm in _perfect_matchings(edge_set, verts):
        guard[0] -= 1
        if guard[0] < 0:
            return None
        rest = _factorize(edge_set - {frozenset(p) for p in pm}, verts, need - 1, guard)
        if rest is not None:
            return [pm] + rest
    return None


def full_round_robin_with(seeds, managers):
    """A full round robin (everyone plays everyone exactly once) that contains each
    seed matching as one of its rounds. Returns [seed rounds..., other rounds...] or
    None if the seeds overlap or it cannot be built quickly."""
    edges = {frozenset(p) for p in itertools.combinations(managers, 2)}
    used, factors = set(), []
    for s in seeds:
        if s is None:
            continue
        se = {frozenset(p) for p in s}
        if se & used:                      # two seeds share a pair: impossible
            return None
        used |= se
        factors.append([tuple(p) for p in s])
    rest = _factorize(edges - used, managers, (len(managers) - 1) - len(factors), [400000])
    return None if rest is None else factors + rest


def _round_rivalry(rnd, hist, person_of):
    return sum(rivalry_score(a, b, hist, person_of) for a, b in rnd)
...
def build_schedule(hist, owners):
    ...
    tops = top_matchings(managers, hist, person_of, 8)
    weekA = _forced_or_die(FORCED_WEEK1, person_of, "Week 1") or tops[0]
    weekB = _forced_or_die(FORCED_THANKS, person_of, "Thanksgiving")
    if weekB is None:                           # optimizer mode: B disjoint from A
        aset = {frozenset(p) for p in weekA}
        weekB = next((m for m in tops if not ({frozenset(p) for p in m} & aset)), None)

    has_thanks = (1 <= THANKS_WEEK <= REG_WEEKS and THANKS_WEEK != 1 and weekB is not None)
    seeds = [weekA, weekB] if has_thanks else [weekA]

    factn = full_round_robin_with(seeds, managers)
    if factn is None:                           # rare safety fallback
        return _build_schedule_simple(weekA, weekB, managers, person_of, hist, has_thanks)

    rivalry_weeks = {1: factn[0]}
    others = factn[2:] if has_thanks else factn[1:]
    if has_thanks:
        rivalry_weeks[THANKS_WEEK] = factn[1]
    others.sort(key=lambda r: _round_rivalry(r, hist, person_of), reverse=True)

    # rivalry weeks fixed; remaining unique rounds fill the rest; any leftover weeks
    # (14 weeks for 12 teams means a few rematches are unavoidable) rerun the most
    # rivalrous regular rounds.
    fill = (others + [others[i % len(others)] for i in range(REG_WEEKS)]) if others else []
    weeks, fi = {}, 0
    for w in range(1, REG_WEEKS + 1):
        if w in rivalry_weeks:
            weeks[w] = rivalry_weeks[w]
        else:
            weeks[w] = fill[fi]; fi += 1

    out = []
    for w in range(1, REG_WEEKS + 1):
        rivalry = w in rivalry_weeks
...
def _build_schedule_simple(weekA, weekB, managers, person_of, hist, has_thanks):
    """Fallback if the seeded factorization cannot be built: dedicated rivalry weeks
    plus a plain round robin for the rest."""
    rr = round_robin(managers)
    rivalry_weeks = {1: weekA}
    if has_thanks:
        rivalry_weeks[THANKS_WEEK] = weekB
    fill = rr + [rr[i % len(rr)] for i in range(REG_WEEKS)]
    weeks, fi = {}, 0
    for w in range(1, REG_WEEKS + 1):
        if w in rivalry_weeks:
            weeks[w] = rivalry_weeks[w]
...
def publish_schedule(league, season, hist, owners):
    print("Roster mapping (Sleeper handle -> resolved person):")
    for info in owners.values():
        flag = ("   <-- UNMAPPED: add this handle to REAL_NAMES"
                if info.get("name") == info.get("handle") else "")
        print(f"  {info.get('handle')} -> {info.get('name')}{flag}")
...
# Draft recap
# ----------------------------------------------------------------------------
DRAFT_PERSONA = """THIS EDITION: the draft recap, written the week after the draft.
Cover the whole draft in depth. Structure:
  <h2> a punchy headline for the draft
  a short <p> intro on the draft as a whole
  <h3>Team by Team</h3> then, for EACH team, an <h3> subhead reading "Team (Grade)"
    with a letter grade from A+ to F, followed by one <p> that covers, in order: the
    team's biggest STRENGTH, its biggest WEAKNESS, one pick to HIGHLIGHT (a smart or
    exciting selection, named), and one QUESTIONABLE pick (a reach or head scratcher,
    named), then a one line verdict. Use the real player names from the briefing.
    Cover every team, no exceptions.
  <h3>Draft Power Rankings</h3> an <ol> ranking ALL teams from the best draft to the
    worst, with one line each on why they land there. This is the definitive order.
  <h3>The Back Page</h3> a season prediction based on these rosters, and a fair,
    slightly grudging word on your own draft.
Ground every pick in the briefing. You may lean on football knowledge to judge the
players, but never invent a pick that is not listed."""


def current_draft(league, owners):
    """Full current-season draft grouped by team. Returns (rows, complete).
    Defensive: ([], False) on any error or if no draft exists yet."""
    try:
        lid, season = league["league_id"], str(league.get("season"))
        drafts = _get(f"league/{lid}/drafts") or []
        if not drafts:
            return [], False
        draft = next((d for d in drafts if str(d.get("season")) == season), drafts[0])
        complete = draft.get("status") == "complete"
        picks = _get(f"draft/{draft['draft_id']}/picks") or []
        by_roster = defaultdict(list)
        for p in picks:
            by_roster[p.get("roster_id")].append(p)
        rows = []
        for rid, plist in by_roster.items():
            info = owners.get(rid, {})
            plist.sort(key=lambda p: (p.get("round") or 0, p.get("pick_no") or 0))
            fmt, counts = [], defaultdict(int)
            for p in plist:
                meta = p.get("metadata") or {}
                nm = f"{meta.get('first_name','')} {meta.get('last_name','')}".strip() or "a pick"
                pos, tm = meta.get("position", ""), meta.get("team", "")
                fmt.append((p.get("round"), nm, pos, tm))
                if pos:
                    counts[pos] += 1
            rows.append({"team": info.get("team", "?"), "person": info.get("name", "?"),
                         "picks": fmt, "counts": dict(counts)})
        return rows, complete
    except Exception as e:
        print(f"current_draft skipped: {e!r}")
        return [], False


def build_draft(league, season, hist, owners):
    rows, complete = current_draft(league, owners)
    if not rows:
        return None
    lines = [f"LEAGUE: {league.get('name')}  SEASON: {season}  EDITION: DRAFT RECAP",
             f"Draft status: {'complete' if complete else 'not marked complete yet'}",
             "", "DRAFT RESULTS BY TEAM:"]
    for r in sorted(rows, key=lambda x: x["person"]):
        picks = "; ".join(f"R{rd} {nm} ({pos}-{tm})" if pos else f"R{rd} {nm}"
                          for rd, nm, pos, tm in r["picks"])
        lines.append(f"\n{r['team']}:")
        lines.append(f"  picks: {picks}")
        if r["counts"]:
            lines.append("  positions drafted: "
                         + ", ".join(f"{p} {n}" for p, n in sorted(r["counts"].items())))
    if hist["alltime"]:
        lines.append("\nALL-TIME CONTEXT (win rates, for framing predictions):")
        lines.append(alltime_block(hist))
    lore = _read_lore()
    if lore.strip():
        lines.append("\nLEAGUE LORE (personalities and history):\n" + lore)
    return "\n".join(lines)


def publish_draft(league, season, hist, owners):
    briefing = build_draft(league, season, hist, owners)
    if not briefing:
        raise SystemExit("No draft found for the current season yet. Once your draft "
                         "shows complete on Sleeper, run the draft column again.")
    body = call_claude(briefing, COMMON_BASE + "\n\n" + BIAS_SLIGHT + "\n\n" + DRAFT_PERSONA)
    date = datetime.now(timezone.utc)
    title, slug = f"{season} Draft Recap", f"draft-{season}"
    (EDITIONS / f"{slug}.html").write_text(render_edition(title, body, date))
    update_manifest(slug, title, date)
    rebuild_index()
    print(f"Published {slug}")
...
def resolve_column_type(week):
    ...
    if ct in ("weekly", "preseason", "schedule", "draft", "recap", "preview"):
        return ct
    return "recap" if _completed_week() else "preseason"
...
def main():
    ...
    if column_type == "schedule":
        ...
        return
    if column_type == "draft":
        publish_draft(league, season, hist, owner_map(LEAGUE_ID))
        return
    if column_type == "preseason":
        briefing, title, slug = build_preseason(league, season, hist)
    elif column_type == "recap":
        briefing, title, slug = build_recap(league, season, hist)
    elif column_type == "preview":
        briefing, title, slug = build_preview(league, season, hist)
    else:
