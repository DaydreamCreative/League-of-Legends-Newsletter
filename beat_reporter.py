...
def build_schedule(hist, owners):
    ...
    label_of = {i["user_id"]: i["team"] for i in owners.values() if i["user_id"]}
    if len(managers) % 2:                       # odd count: add a bye
        managers.append(None)
        person_of[None] = "BYE"; label_of[None] = "BYE (no game this week)"
    ...
    for w in range(1, REG_WEEKS + 1):
        ...
        games = [(label_of.get(a, "?"), label_of.get(b, "?"),
